...
function normalizeParameters(
  ...
  for (const {
    ...
    if (bodyParameter) {
      context.parameterMap.set(bodyParameter, {
        ...
        location: camelCasify(bodyParameter.name),
      });
    }
  }

  function parametersOf(operation: OperationDetails) {
    const theseParameters = operation.parameters.parameters.map(
      ({ param }) => param
    );
    if (operation.parameters.bodyParameter)
      theseParameters.push(operation.parameters.bodyParameter);
...
export function writePositionals(
  ...
  if (details.parameters.bodyParameter) {
    const { typeName } = context.parameterMap.get(
      details.parameters.bodyParameter
    )!;
    positionalParams.push(
...
function convertToTypeScript(
  ...
  if (t.kind === "Model" && t.indexer) {
    if (isNeverType(t.indexer.key)) {
    } else {
      const name = getIntrinsicModelName(context.program, t.indexer.key);
      if (name === "string") {
        return `Record<string, ${convertToTypeScript(
          context,
          t.indexer.value!
        )}>`;
  ...
  switch (t.kind) {
    ...
    case "Model":
      context.cachedTypeScriptTypeNames.set(
        t,
        t.name ?? preferredAlternativeName
      );
...
function addInterface(
  ...
  const t = getEffectiveModelType(context.program, tOriginal);
...
function addEnum(context: WriterContext, e: Enum): string {
  ...
  const variants = [...e.members.values()]
    .map(({ name, value }) =>
      typeof value === "string"
        ? `"${value}"`
        : typeof value === "number"
